...
/// Every diagnostic the checker can produce, as a method on any error reporter.
pub(crate) trait SemanticError: ErrorReporter {
    ...
    fn instantiation_too_deep(&self, pos: Pos, limit: usize) {
        self.report(
            pos,
            format!("The type instantiation is too deep (the limit is {limit} nested type arguments)"),
        );
    }
    ...
    fn binop_on_untyped_float(&self, pos: Pos, op: impl Display) {
        self.report(
            pos,
            format!("Cannot perform {op} binary operation on floating number"),
        );
    }
